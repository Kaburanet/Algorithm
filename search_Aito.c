...
void count_have_same_value(int M, int N, char *strBuffer_data, SameValue **array_write)
{
    ...
    for (int i = 0; i < M; i++)
    {
        for (int j = 0; j < N; j++)
        {
            ...
            if (((int)strBuffer_data[pos_other] == strBuffer_data[pos_A]) && (pos_A != pos_other))
            {
...
// 範囲を調べて一致する人数を取得する
ANSWER analyzeRange(char *model, SameValue **same_value_array, char *strBuffer_data, int M)
{
...
int main(int argc, char *argv[])
{
    ...
    int M = (int)(chL_pos - strBuffer_data) + 1; // アンケートの質問数(改行列含む)
    int N = strlen(strBuffer_data) / M;          // 回答人数
    ...
    {
        ans = analyzeRange(line, same_value_array, strBuffer_data, M);                           // 答えを取得
    ...
    //****************************************************************結果の出力***********************************************************
    save_data[strlen(save_data) - 1] = '\0';
    save_to_file(argv[3], save_data);
    // printf("%s\n", save_data);
