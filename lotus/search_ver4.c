// ver4の追加内容
// それぞれのstart、lengthにおける配列の位置を保持
// 走査するstart、lengthで既に保持しているstartが同じで最もlengthが小さい方向から近い配列を利用することで時短
// とてつもないメモリ使用量でかえって時間がかかってしまった
...
void count_have_same_value(int M, int N, char *strBuffer_data, SameValue **array_write)
{
  ...
  for (int i = 0; i < M; i++) // アンケートの質問番号によるループ
  {
    for (int j = 0; j < N; j++) // 回答人数によるループ
    {
      ...
      {
        array_write[i]->vec[0][m] = pos_A; // i個目の質問でm番目に同じ回答をした人の回答番号
        array_write[i]->vec[1][m] = j;     // i個目の質問でm番目に同じ回答をした人の位置
        array_write[i]->m += 1;            // i個目の質問で同じ値を持つ回答者数の更新
      }
    }
  }
}
  ...
  if (match_value->ans[start][length].start == 0) // 既に同じstart、lengthで実施しているかで分岐
  {
    ...
    if (match_value->ans[start][m_nearest_length].match_count == 0 || match_value->ans[start][m_nearest_length].match_count > min)
    {
      ...
      { // 1回目のループは見つかった最小のmに対してそのmの項目を全部を調べるために行う
        ...
        {
          ans.match_count++;
          match_value->match_array[start][length][ans.match_count] = y;
        }
      }
    }
    else
